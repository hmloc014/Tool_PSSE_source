...
class MyForm(wx.Frame):
    ...
    def __init__(self):
        ...
        self.textCtrl_ID = wx.TextCtrl( panel, -1, '', pos=(0, 210)) #, size=wx.Size( 120,-1 ), 0 )
    ...
    def onSelectToMove(self, event):
        ...
        for i in range(len(move)):
            ...
            obj = self.lb2.GetClientData(move[i])
    ...
    def oneAdd_Fcn(self,event):
        print('This is oneAdd',a[0])
        for i in range(len(a)):
            obj = self.lb.GetClientData(a[i])
            print("obj: ",obj)
            print("obj.make: ",obj.make)
            if not obj.make in self.lb2.Items:
                self.lb2.Append(obj.make, obj)
    ...
    def multipleAdd_Fcn(self,event):
        ...
        for i in range(len(b)):
            obj = self.lb.GetClientData(i)
            print("obj.model: ",obj.model)
            print("obj.make: ",obj.make)
            if not obj.make in self.lb2.Items:
                self.lb2.Append(obj.make, obj.model)
    ...
    def oneMove_Fcn(self,event):
        ...
        for i in range(len(move)):
            print("len move is: ",len(move))
            print("i is: ",i)
            obj = self.lb2.GetClientData(len(move)-1-i)
            print("obj: ",obj)
    ...
    def onTextID( self, event ):
        searchText = self.textCtrl_ID.GetValue()
        items = ['Camaro','F7Z','punto','370Z','Hang','Khoa','Hoa','Chau','Thao']
        print(items)
        print(searchText)
        result = []   
