...
class MyGrid(wx.grid.Grid):
    def __init__(self, parent):
        wx.grid.Grid.__init__(self, parent, wx.ID_ANY, wx.DefaultPosition,  wx.Size( 1635,200 ), 0)
        ...
        self.history = []
        self.parent = parent
        print("___________________________SELF is:________________________",self)
    ...
    def on_key(self, event):
        ...
        print("--------------event.ControlDown() and event.GetKeyCode()==13----------",event.ControlDown() and event.GetKeyCode()==13)
        ...
        # Ctrl+V or Shift + Insert
            ...
            # start_row, start_col, end_row, end_col = self.get_selection()
            # if (start_col==end_col and start_row==end_row):
            #     row == len(matrixBus)
            #     self.AddNewBus(busNum)
    ...
    def get_selection(self):
        ...
        """
        global row,col,cellValue,cellVal,busNum,busName,busArea,busZone,busBaseKV,busVM,busVA
        ...
        if (start_col==end_col and start_row==end_row):
            row = start_row
            col = start_col
            print("-----------------Row in onGridSelectCell3 is:",row)
            colLabel = self.GetColLabelValue(col)
            cellValue = self.GetCellValue(row,col)
            cellVal = self.GetCellValue(row-1,col)
            print("-----------------cellValue in onGridSelectCell3 is:",cellValue)
            busNum = int(self.GetCellValue(row,0))
            busName = self.GetCellValue(row,1)
            busArea = int(self.GetCellValue(row,3))
            busZone = int(self.GetCellValue(row,5))
            busBaseKV = float(self.GetCellValue(row,2))
            busVM = float(self.GetCellValue(row,10))
            busVA = float(self.GetCellValue(row,11))
